...
package app.okaneko.util.envConfig

import app.okaneko.util.envConfig.exceptions.KeyNotFoundException
import app.okaneko.util.envConfig.exceptions.NotInitializedException
import io.ktor.server.config.*

// Taken from https://github.com/Olaren15/ktor2-env-config
/**
 * Environment config main class
 *
 * Allows the usage of different configs base on the current environment
 */
object EnvConfig {
    ...
    fun getInt(propertyKey: String): Int =
        getIntInternal(environment, propertyKey)
            ?: throw KeyNotFoundException("Property $propertyKey does not exist")
    ...
    fun getList(propertyKey: String): List<String> =
        getListInternal(environment, propertyKey)
            ?: throw KeyNotFoundException("Property $propertyKey does not exist")
    ...
    private fun getPropertyInternal(environment: String, propertyKey: String): ApplicationConfigValue? {
        // do we have a config?
        if (!EnvConfig::config.isInitialized) {
